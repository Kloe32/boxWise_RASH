...
describe("authService.getDetail", () => {
  it("should throw 404 when user not found", async () => {
    ...
    await expect(authService.getDetail(999)).rejects.toMatchObject({
      statusCode: 404,
    });
  });

  it("should return user detail", async () => {
    mockAuthRepo.findUserById.mockResolvedValue(fakeUser);
    const result = await authService.getDetail(1);
...
describe("authService.updateUser", () => {
  it("should throw 404 when user not found", async () => {
    mockAuthRepo.findUserById.mockResolvedValue(null);
    await expect(
      authService.updateUser(999, { full_name: "X" }),
    ).rejects.toMatchObject({ statusCode: 404 });
  });

  it("should throw 400 when no valid fields provided", async () => {
    mockAuthRepo.findUserById.mockResolvedValue(fakeUser);
    await expect(
      authService.updateUser(1, { foo: "bar" }),
    ).rejects.toMatchObject({ statusCode: 400 });
  });
  ...
  it("should throw 400 for invalid role", async () => {
    mockAuthRepo.findUserById.mockResolvedValue(fakeUser);
    await expect(
      authService.updateUser(1, { role: "SUPERADMIN" }),
    ).rejects.toMatchObject({ statusCode: 400 });
  });

  it("should update valid fields and return updated user", async () => {
    const updated = { ...fakeUser, full_name: "Jane" };
    mockAuthRepo.findUserById
      .mockResolvedValueOnce(fakeUser) // first call (existence check)
      .mockResolvedValueOnce(updated); // second call (return updated)
    mockAuthRepo.updateUserById.mockResolvedValue([1]);

    const result = await authService.updateUser(1, { full_name: "Jane" });
    expect(mockAuthRepo.updateUserById).toHaveBeenCalledWith(1, {
      full_name: "Jane",
    });
